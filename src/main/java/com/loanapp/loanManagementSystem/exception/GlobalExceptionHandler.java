...
@RestControllerAdvice(basePackages = "com.loanapp.loanManagementSystem")
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorDetails> handleResourceNotFoundException(ResourceNotFoundException e, WebRequest request) {
        ...
        log.warn("ResourceNotFound : " + e.getMessage(),e);
    ...
    public ResponseEntity<?> handleInvalidLoanStateException(InvalidLoanStateException e, WebRequest request){
        ErrorDetails details=new ErrorDetails(HttpStatus.NOT_FOUND,e.getMessage(),request.getDescription(false),LocalDateTime.now());
        log.warn("InvalidLoanState : " + e.getMessage(), e);
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(details);
    }
